---
tags:
  - internship
  - fcn
date: 1 / 20 / 26
hours: "4.5"
---
Back at it with the phones. We provisioned about four more boxes today, which is starting to feel like a well-oiled routine at this point. Unbox, connect, boot, label, done. The Lynden installation is going to go very smoothly at this rate!

The more exciting part of the day was getting introduced to Cisco switches. We started unboxing and setting up a new batch, and that's where I learned about stack cables, which are used to link multiple switches together so they behave as a single unit. It's one of those concepts that sounds simple but clicks in a really satisfying way once you see it physically in front of you.

From there I got to shadow Ryan again as he walked through adopting the switches onto the network and configuring things like VLANs. It's a different layer of the infrastructure than anything I'd touched before, and watching how carefully each setting gets mapped out gave me a new appreciation for how much thought goes into a network that just... works.

The cherry on top was a tour of the main building's server room. There's something about walking into a room full of blinking lights and humming hardware that feels like seeing behind the curtain. This is where it all lives. Week three is off to a good start.